...
describe("Bridge Contract", function () {
  ...
  it("should deposit and verify merkle proof", async () => {
    ...
    expect(await bridgeContract.verifyMerkleProof(tokenAddress, amount, originalNetwork, destinationNetwork, destinationAddress, proof, index, rootSC)).to.be.true;
  ...
  it("should add a withdraw function in the bridge smart contract", async () => {
    ...
    // verify merkle proof
    expect(verifyMerkleProof(leafValue, proof, index, rootJS)).to.be.true;
    expect(await bridgeContract.verifyMerkleProof(tokenAddress, amount, originalNetwork, destinationNetwork, destinationAddress, proof, index, rootJS)).to.be.true;

    // withdraw 
    const lastGlobalExitRootNum = await bridgeContract.lastGlobalExitRootNum();

    // Can't withdraw without tokens
    await expect(bridgeContract.withdraw(tokenAddress, amount, originalNetwork, destinationNetwork, destinationAddress,
      proof, index, lastGlobalExitRootNum, mainnetExitRoot, rollupExitRootSC)).to.be.revertedWith("ERC20: transfer amount exceeds balance")
    ...
    await expect(bridgeContract.withdraw(tokenAddress, amount, originalNetwork, destinationNetwork, destinationAddress,
      proof, index, lastGlobalExitRootNum, mainnetExitRoot, rollupExitRootSC))
    ...
    await expect(bridgeContract.withdraw(tokenAddress, amount, originalNetwork, destinationNetwork, destinationAddress,
      proof, index, lastGlobalExitRootNum, mainnetExitRoot, rollupExitRootSC)).to.be.revertedWith("Bridge:withdraw: ALREADY_CLAIMED_WITHDRAW")
  });
