...
const deployParameters = require("./deploy_ger_parameters.json");
...
async function main() {
    ...
    const GlobalExitRootSovereignFactory = await ethers.getContractFactory(
        "GlobalExitRootManagerL2SovereignChain",
        deployer
    );

    const bridgeAddress = ethers.ZeroAddress;
    const globalExitRootSovereignContract = await GlobalExitRootSovereignFactory.deploy(bridgeAddress);
    await globalExitRootSovereignContract.waitForDeployment();

    // print contract address deployed
    console.log("\n#######################");
    console.log("globalExitRootSovereignContract deployed to:", globalExitRootSovereignContract.target);
    console.log("#######################\n");
    // print verification command line
    console.log("you can verify the contract address with:");
    console.log(
        `npx hardhat verify --constructor-args upgrade/arguments.js ${globalExitRootSovereignContract.target} --network ${process.env.HARDHAT_NETWORK}\n`
    );
