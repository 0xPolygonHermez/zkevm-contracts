...
async function main() {
    logger.info('Starting tool to add OP Succinct config');
    ...
    const destPath = params.outputPath
        ...
        : path.join(__dirname, `add_op_succinct_config_output_${params.type}_${dateStr}.json`);
    ...
    const { type, rollupAddress, configName, rollupConfigHash, aggregationVkey, rangeVkeyCommitment } = params;
    ...
    const addArgs = [configName, rollupConfigHash, aggregationVkey, rangeVkeyCommitment];

    if (type === transactionTypes.TIMELOCK) {
        logger.info('Creating timelock tx to add OP Succinct config....');
        ...
        const operation = genOperation(
            rollupAddress,
            0, // value
            AggchainFEPFactory.interface.encodeFunctionData('addOpSuccinctConfig', addArgs),
        ...
        logger.info('Creating calldata to add OP Succinct config from multisig...');
        const txAddOpSuccinctConfig = AggchainFEPFactory.interface.encodeFunctionData('addOpSuccinctConfig', addArgs);
        outputJson.rollupAddress = rollupAddress;
        outputJson.configName = configName;
        ...
        outputJson.txAddOpSuccinctConfig = txAddOpSuccinctConfig;
    } else {
        logger.info('Send tx to add OP Succinct config...');
        ...
        logger.info(`Sending addOpSuccinctConfig transaction to AggchainFEP ${rollupAddress}...`);
        try {
            const tx = await aggchainFEP.addOpSuccinctConfig(
                configName,
                rollupConfigHash,
                aggregationVkey,
                rangeVkeyCommitment,
