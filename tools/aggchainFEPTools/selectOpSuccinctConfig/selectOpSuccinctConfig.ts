...
async function main() {
    logger.info('Starting tool to select OP Succinct config');
    ...
    const destPath = params.outputPath
        ...
        : path.join(__dirname, `select_op_succinct_config_output_${params.type}_${dateStr}.json`);
    ...
    const mandatoryParameters = ['type', 'rollupAddress', 'configName'];
    ...
    const { type, rollupAddress, configName } = params;
    ...
    if (type === transactionTypes.TIMELOCK) {
        logger.info('Creating timelock tx to select OP Succinct config....');
        ...
        const operation = genOperation(
            rollupAddress,
            0, // value
            AggchainFEPFactory.interface.encodeFunctionData('selectOpSuccinctConfig', [configName]),
        ...
        logger.info('Creating calldata to select OP Succinct config from multisig...');
        const txSelectOpSuccinctConfig = AggchainFEPFactory.interface.encodeFunctionData('selectOpSuccinctConfig', [
            configName,
        ]);
        outputJson.rollupAddress = rollupAddress;
        outputJson.configName = configName;
        outputJson.txSelectOpSuccinctConfig = txSelectOpSuccinctConfig;
    } else {
        logger.info('Send tx to select OP Succinct config...');
        ...
        logger.info(`Sending selectOpSuccinctConfig transaction to AggchainFEP ${rollupAddress}...`);
        try {
            const tx = await aggchainFEP.selectOpSuccinctConfig(configName);
            await tx.wait();
            outputJson.rollupAddress = rollupAddress;
            outputJson.configName = configName;
