...
import "./AgglayerBridgeL2Module.sol";
...
contract AgglayerBridgeL2 is
    AgglayerBridge,
    AgglayerBridgeL2Storage,
    IAgglayerBridgeL2
{
    using SafeERC20 for ITokenWrappedBridgeUpgradeable;

    // Embedded in implementation bytecode, so changing modules requires a proxy upgrade.
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    AgglayerBridgeL2Module internal immutable bridgeModule;
    ...
    constructor() AgglayerBridge() {
        deployer = msg.sender;
        bridgeModule = new AgglayerBridgeL2Module();
    ...
    function setMultipleSovereignTokenAddress(
        ...
        // Keep the ABI entrypoint here; the module checks the original caller's role.
        _delegateToBridgeModule();
    }
     ...
     * @param _bridgeManager Bridge manager address
     */
    function setBridgeManager(address _bridgeManager) external virtual {
        _delegateToBridgeModule();
    }
    ...
    // Only for matching external selectors with no return values; not internal dispatch.
    /// @custom:oz-upgrades-unsafe-allow delegatecall
    function _delegateToBridgeModule() internal {
        (bool success, bytes memory returnData) = address(bridgeModule)
            .delegatecall(msg.data);
        if (!success) {
            // Preserve custom errors and revert strings exactly across the module boundary.
