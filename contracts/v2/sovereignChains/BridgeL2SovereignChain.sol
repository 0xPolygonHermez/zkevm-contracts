    ...
    IBridgeL2SovereignChains
{
    ...
    function bridgeAsset(
        ...
        if (token == address(0)) {
            ...
            originNetwork = gasTokenNetwork; // q: always 0?
            originTokenAddress = gasTokenAddress; // q: always 0?
            ...
            // q: what if I call this function to transfer weth to another network (not mainnet)?
            if (token == address(WETHToken)) {
                // If weth is not mintable, don't use burn but transfer to bridge
                if (wrappedAddressIsNotMintable[token]) {
    ...
    function bridgeMessageWETH(
        ...
        // If weth is not mintable use transfer instead of burn
        if (wrappedAddressIsNotMintable[address(WETHToken)]) {
            // Transfer wETH
            IERC20Upgradeable(address(WETHToken)).safeTransferFrom(
                msg.sender,
                address(this),
                amountWETH
            );
    ...
    function claimAsset(
        ...
        if (originTokenAddress == address(0)) {
            if (address(WETHToken) == address(0)) {
                ...
                if (wrappedAddressIsNotMintable[address(WETHToken)]) {
                    // Transfer wETH
                    IERC20Upgradeable(address(WETHToken)).safeTransfer(
                        destinationAddress,
                        amount
                    );
                } else {
            ...
            if (
                ...
                if (originNetwork == networkID) {
                    ...
                    if (wrappedToken == address(0)) {
                        ...
                        if (wrappedAddressIsNotMintable[wrappedToken]) {
                            // Transfer tokens
                            IERC20Upgradeable(wrappedToken).safeTransfer(
                                destinationAddress,
                                amount
                            );
                        } else {
    ...
    function claimMessage(
        ...
        if (address(WETHToken) == address(0)) {
            ...
            // If is not mintable use transfer instead of mint
            if (wrappedAddressIsNotMintable[address(WETHToken)]) {
                // Transfer wETH
                IERC20Upgradeable(address(WETHToken)).safeTransfer(
                    destinationAddress,
