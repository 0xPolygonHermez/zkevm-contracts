# Upgrade Sovereign Bridge (Native Blocked)

Script to create schedule and execute transaction for upgrading bridge L2 to the native-blocked sovereign version.

- `AgglayerBridgeL2FromEtrog` (or previous version) --> `AgglayerBridgeL2NativeBlockedFromEtrog`

This upgrade blocks native gas token deposits and claims on L2 bridges where the gas token is not ether (i.e. `WETHToken != address(0)`). Ether-native bridges are unaffected.
...
```bash
npx hardhat run ./upgrade/upgradeFromEtrogToBlockedNative/upgradeSovereignBridge.ts --network <network>
```
