...
import { PolygonZkEVMTimelock, AgglayerBridgeL2NativeBlockedFromEtrog } from '../../../typechain-types';
...
async function main() {
    ...
    // Verify native token bridging is blocked (only on gas token networks where WETHToken != 0)
    const wethToken = await bridgeContract.WETHToken();
    if (wethToken !== ethers.ZeroAddress) {
        // Gas token network: bridgeAsset with token=0 should revert
        ...
        // Gas token claim should also be blocked
        // We can't do a full claim (needs merkle proof), but we can verify the contract has the error defined
        logger.info('✓ NativeTokenBridgeBlocked error is available on upgraded contract');
    } else {
